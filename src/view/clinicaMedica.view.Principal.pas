...
type
  TfrmPrincipal = class(TForm)
    ...
    dbExames: TDBGrid;
    Label3: TLabel;
    btnRelExamesTecnicos: TSpeedButton;
    ...
    procedure btnRelExamesTecnicosClick(Sender: TObject);
...
begin
  ...
  dm.qryExames.SQL.add
    ('select u.nome_user, e.* , conv.nome_conv, p.nome_paci from exames as e inner join usuarios as u ON u.id_func_user = e.id_med inner join convenios as conv on e.id_conv = conv.id_conv inner join pacientes as p on e.id_paci = p.id_paci ');
...
procedure TfrmPrincipal.btnRelExamesTecnicosClick(Sender: TObject);
begin
  frmRelExames := TfrmRelExames.Create(nil);
  frmRelExames.showmodal;
end;
