...
begin
  ...
  if Key = #27 then
  begin
    if MessageDlg('Deseja encerrar a aplicação?', mtInformation, [mbYes, MbNo],
      0) = mrYes then
    begin
...
begin
  ...
  begin
    MessageDlg('Campo Usuário está vazio!', mtInformation, [mbOK], 0);
    exit;
  end;
  ...
  begin
    MessageDlg('Usuário e/ou senha estão incorretos!', mtInformation,
      [mbOK], 0);
    edtUser.Clear;
    edtSenha.Clear;
    edtUser.SetFocus;
